...
components/lotto/
  ...
  AiRecommendationCard.tsx      # 신규 — 번호+별점+근거 카드
...
| `random` | 종합 랜덤 | (frequency + elite + balanced + cold) 4개 전략 가중치 평균 | "4개 전략 가중 평균 랜덤" |
...
- `analysis.test.ts`: 별점 범위(1~5), 근거 문구 생성 조건, 후보군 12/12/8 산출 로직.
